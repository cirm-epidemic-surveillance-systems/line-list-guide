---
title: "Estimating delays from line list data with noisy symptom onset date"
---
...
```{r}
# RNG for negative binomial distribution, mean and overdispersion parameterization
rnbinom2 <- function(n, mu, inv_rho) {
  rho = 1/inv_rho
  rnbinom(n, size = rho, prob = rho / (mu + rho))
}

var(rnbinom2(10000, mu = 5, inv_rho = 1))
...
ll_all <- bind_rows(list(
  "0" = ll_clean,
  "0.1" = ll_clean |> date_noise(noise_func = \(n, delay) {rnbinom2(n = n, mu = delay, inv_rho = 0.1)}),
  "0.5" = ll_clean |> date_noise(noise_func = \(n, delay) {rnbinom2(n = n, mu = delay, inv_rho = 0.5)}),
  "1" = ll_clean |> date_noise(noise_func = \(n, delay) {rnbinom2(n = n, mu = delay, inv_rho = 1)}),
  "2" = ll_clean |> date_noise(noise_func = \(n, delay) {rnbinom2(n = n, mu = delay, inv_rho = 2)})
  ),
  .id = "ll_source")
```
...
c(delay_admission_estimates, delay_admission_estimates_natural) |> 
  ...
  mutate(ll_source = as.numeric(ll_source)) |> 
  ...
  labs(y = "Recall noise factor", x = "Value") +
  ggtitle("Delay between date of onset and date of admission") +
  theme_bw() +
