...
### Messy analysis: Use onset of first symptoms (mild or severe)
```{r}
ll_messy <- copy(ll)
do_coalesce(ll_messy , "date_onset", c("date_onset_mild", "date_onset_severe"))
...
### Correct analysis: Use only onset dates for severe symptoms
```{r}
delay_data <- ll |> 
  filter(!is.na(date_onset_severe), !is.na(date_admission)) |>
  mutate(delay = as.numeric(date_admission - date_onset_severe)) |>
  filter(delay >= 0) |>
  group_by(delay) |>
  summarise(n = n()) |>
  ungroup() |>
...
results_df <- data.frame(
  Estimate = orcats::fct_inorder(c("Inconsistent definition", "Consistent definition")),
...
ggsave(here::here("vignettes", "figures", "issue_symptom_definition.png"), width = 10, height = 6)
```
