...
Perfect data
```{r}
ll_perfect <- get_true(ll)

delay_data <- ll_perfect |> 
  mutate(delay = as.numeric(date_admission - date_onset)) |>
  filter(delay >= 0) |>
  group_by(delay) |>
  summarise(n = n()) |>
...
pcd_fit_perfect <- pcd_model$sample(
  data = pcd_data_function(delay_data),
  chains = 4,
  parallel_chains = 2, # Run 2 chains in parallel
  threads_per_chain = 2, # Use 2 cores per chain
...
## Results
```{r}
pcd_fit_messy$summary(c("params[1]", "params[2]"))
pcd_fit_correct$summary(c("params[1]", "params[2]"))
pcd_fit_perfect$summary(c("params[1]", "params[2]"))
```


```{r}
rvars <- lapply(list(pcd_fit_messy, pcd_fit_correct, pcd_fit_perfect), function(x) posterior::as_draws_rvars(x$draws("params")))

results_df <- data.frame(
  Estimate = forcats::fct_inorder(c("Ad-hoc resolution", "Model with censoring", "Exact dates available")),
...
ggplot(results_df) +
  ...
  scale_color_manual(values = c("#800000", "#125c87", "darkgreen")) +
  scale_fill_manual(values = c("#a00000", "#1a80bb", "darkgreen")) +
  theme_bw() +
